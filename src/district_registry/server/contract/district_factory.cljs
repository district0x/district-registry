...
(defn create-district [{:keys [:creator :info-hash]} & [opts]]
  (contract-call :district-factory :create-district creator info-hash (merge {:gas 3000000} opts)))

(defn create-district-data [{:keys [:creator :info-hash]}]
  (web3-eth/contract-get-data (instance :district-factory) :create-district creator info-hash ))
