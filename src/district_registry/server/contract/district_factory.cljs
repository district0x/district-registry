...
(defn create-district [{:keys [:creator :info-hash :dnt-weight]} & [opts]]
  (contract-call :district-factory :create-district creator info-hash dnt-weight (merge {:gas 3000000} opts)))

(defn create-district-data [{:keys [:creator :info-hash :dnt-weight]}]
  (web3-eth/contract-get-data (instance :district-factory) :create-district creator info-hash dnt-weight))
